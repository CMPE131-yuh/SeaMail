...
def sendEmail():
    if request.method == 'POST':
        ...
        rec = request.form['recipe']
        emails.insert_one(
            {'username': rec, 'sender': session['user'], 'subject': sub, 'message': msg, 'delete': False})
...
def addTodo():
    if request.method == 'POST':
        ...
        todos.insert_one(
            {'username': session['user'], 'item': todoitem, 'delete': False})
...
def logout():
    todolist = todos.find({'username': session['user']})
    maillist = emails.find({'username': session['user']})
    todos_list = list(todolist)
    mails_list = list(maillist)
    ...
    return render_template('logout.html', current_user=session['user'], todos=todos_list, mails=mails_list)
...
def register():
    if request.method == 'POST':
        ...
        new_user = {'username': new_username, 'password': new_user_password,
                    'email': new_user_email, 'blocked': False}
